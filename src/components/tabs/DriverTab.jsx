...
export default function DriverTab({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-50 border border-slate-300 rounded-2xl shadow p-5">
        <h2 className="font-semibold text-lg mb-4">司机端 · 晚餐区</h2>
        <DriverTable orders={tempDriverOrders} onTogglePaymentMethod={updatePaymentMethod} driverMap={driverMap} temp />
        {tempDriverOrders.length === 0 && <div className="text-sm text-slate-500 mt-3">当前筛选条件下暂无晚餐订单。</div>}
      </div>
    </div>
  );
}
